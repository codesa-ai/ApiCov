...
echo "Debug: Argument 5 (xml flag): $5"
...
if [[ -f "$SCRIPT_DIR/apicov" ]]; then
  ...
  if [ -z "$3" ]; then
    ...
    echo "Usage: $0 <root_path> <api_key> <install_dir> [doxygen_path] [--xml|--no-xml]"
    exit 1
  fi

  if [ -n "$4" ]; then
    echo "Debug: Using install_dir: $3 and doxygen_path: $4"
    CMD="$SCRIPT_DIR/apicov \"$1\" \"$2\" --install_dir \"$3\" --doxygen_path \"$4\" $5"
  else
    echo "Debug: Using install_dir: $3"
    CMD="$SCRIPT_DIR/apicov \"$1\" \"$2\" --install_dir \"$3\" $4 $5"
  fi
